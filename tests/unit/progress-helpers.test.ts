...
describe('progress-helpers', () => {
  describe('getStageProgress', () => {
    it('should return correct progress for extracting stage', () => {
      expect(getStageProgress('extracting')).toBe(25);
    });

    it('should return correct progress for analyzing stage', () => {
      expect(getStageProgress('analyzing')).toBe(50);
    });

    it('should return correct progress for saving stage', () => {
      expect(getStageProgress('saving')).toBe(90);
    });

    it('should return correct progress for uploading stage', () => {
      expect(getStageProgress('uploading')).toBe(10);
    });

    it('should return correct progress for ready-to-upload stage', () => {
      expect(getStageProgress('ready-to-upload')).toBe(100);
    });

    it('should return correct progress for complete stage', () => {
      expect(getStageProgress('complete')).toBe(100);
    });

    it('should return 0 progress for error stage', () => {
      expect(getStageProgress('error')).toBe(0);
    });

    it('should return correct progress for rate-limited stage', () => {
      expect(getStageProgress('rate-limited')).toBe(10);
    });

    it('should return correct progress for retrying stage', () => {
      expect(getStageProgress('retrying')).toBe(10);
    });
  });

  describe('getStageMessage', () => {
    it('should return message for extracting stage', () => {
      expect(getStageMessage('extracting')).toBe('Extracting file content...');
    });

    it('should return message for analyzing stage', () => {
      expect(getStageMessage('analyzing')).toBe('Analyzing with AI...');
    });

    it('should return message for saving stage', () => {
      expect(getStageMessage('saving')).toBe('Saving analysis...');
    });

    it('should return message for uploading stage', () => {
      expect(getStageMessage('uploading')).toBe('Uploading to Evernote...');
    });

    it('should return message for ready-to-upload stage', () => {
      expect(getStageMessage('ready-to-upload')).toBe('Analysis complete, ready to upload');
    });

    it('should return message for complete stage', () => {
      expect(getStageMessage('complete')).toBe('Uploaded successfully');
    });

    it('should return message for error stage', () => {
      expect(getStageMessage('error')).toBe('Processing failed');
    });

    it('should return message for rate-limited stage without duration', () => {
      expect(getStageMessage('rate-limited')).toBe('Rate limited');
    });

    it('should return message for rate-limited stage with duration', () => {
      expect(getStageMessage('rate-limited', 60)).toBe('Rate limited - retry in 60s');
    });

    it('should return message for retrying stage', () => {
      expect(getStageMessage('retrying')).toBe('Retrying upload...');
    });
  });
