...
**File:** `electron/config/runtime-config.ts`

**Purpose:** Bridge between `.env` file and bundled application code

**Generation:** Auto-generated by `scripts/generate-config.js` during:
- `npm run dev` (via predev hook)
- `npm run build`

**Git Status:** Gitignored (contains sensitive credentials)

**Contents:**
```typescript
export const EVERNOTE_CONSUMER_KEY = 'from-.env-file';
export const EVERNOTE_CONSUMER_SECRET = 'from-.env-file';
export const EVERNOTE_ENDPOINT = 'from-.env-file';
```

**Used By:**
- `electron/evernote/client.ts`
- `electron/evernote/oauth-helper.ts`
- `electron/evernote/note-augmenter.ts`

**Testing:** Tests use module aliasing in `vitest.config.ts` to redirect imports to `tests/mocks/runtime-config-values.ts`, allowing tests to run without the `.env` file or generated config (required for CI environments).
